"""Standalone Vehicle-only P1 at output stride 8 (45x80).

Does not instantiate RSU/Drone, Gaussians, or detection heads.
...
_V90_PREFIX_MAP: Tuple[Tuple[str, str], ...] = (
    ("frontend.encoders.vehicle.", "encoder."),
    ("highres.vehicle.", "highres."),
    ("heatmap_heads.vehicle.", "heatmap_head."),
    ("depth_heads.vehicle.", "depth_head."),
    ("depth_moments.vehicle.", "depth_moments."),
)


def remap_v90_vehicle_key(key: str) -> str:
    """Map a V90 Vehicle P1 state_dict key onto this standalone model.

    Args:
        key: Checkpoint tensor name, without a ``module.`` prefix.

    Returns:
        Remapped name, or ``""`` if the key is not a Vehicle P1 tensor.
    """
...
class VehicleV45P1(nn.Module):
    """Vehicle CamEncode → R2 stride-2 Conv → HighResFusion → P1 heads.

    Args:
        args: ``hypes['model']['args']``. Reads Vehicle ``cam`` and optional
            ``output_stride`` (default 8). RSU/Drone blocks are ignored.
    """

    def __init__(self, args: Dict[str, Any]) -> None:
        super().__init__()
        cam_cfg = args["vehicle"]["cam"]
        ...
        self.encoder = build_camencode(cam_cfg, "vehicle")
        ...
        print(
            "[V45] Vehicle-only P1: CamEncode R2/F45, Conv3x3 stride-2 on R2, "
            f"HighResFusion/heads at {self.feat_h}x{self.feat_w} "
            f"(stride={self.output_stride})"
        )

    def train(self, mode: bool = True) -> "VehicleV45P1":
    ...
    def assert_train_eval_state(self, mode: bool) -> None:
        """Assert the Vehicle CamEncode BN-freeze / fine-tune contract."""
        assert_camencode_train_eval_state(self.encoder, mode, "vehicle")
    ...
    def forward_features(
        ...
        Args:
            imgs: Vehicle camera batch, ``[B_a, V, C, H, W]`` or ``[N, C, H, W]``.
    ...
    def forward(self, data_dict: Dict[str, Any]) -> Dict[str, Dict[str, torch.Tensor]]:
        """Predict Vehicle heatmap and categorical depth at stride 8.

        Args:
            data_dict: ``batch['ego']``. Only ``vehicle`` cameras are read.

        Returns:
            ``{'vehicle': {heatmap_logits, depth_logits, depth_z_mean,
            depth_z_var}}``.
        """
        payload = data_dict["vehicle"]
        ...
        payload.update(vehicle)
        return {"vehicle": vehicle}

    def load_v90_vehicle_weights(
        self, path: str, device: torch.device
    ) -> Dict[str, List[str]]:
        """Load matching V90 Vehicle P1 tensors. New R2 conv stays random.

        Only copies keys whose remapped name exists and whose shape matches.
        Does not call a blind ``strict=False`` load of the raw checkpoint.
        ...
        for src_key, value in state.items():
            dst_key = remap_v90_vehicle_key(src_key)
            if not dst_key:
        ...
        print(f"Loaded V90 Vehicle P1 weights from {path} (epoch={epoch})")
        ...
        if mismatched:
            for item in mismatched:
                print(f"    {item}")
        print(f"  skipped (non-vehicle / unmapped): {len(skipped)}")
        return report

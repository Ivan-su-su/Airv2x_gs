...
class ImageFrontend(nn.Module):
    ...
    def apply_train_eval_state(self, mode: bool) -> None:
        ...
        Heatmap-only finetune (``freeze_backbone=True``) keeps trunk and
        up1/up2 in eval with ``requires_grad=False``.
    ...
    def assert_train_eval_state(self, mode: bool) -> None:
        ...
        for agent_type, encoder in self.encoders.items():
            expected_encoder_train = bool(mode) and not self.freeze_backbone
            if bool(encoder.training) != expected_encoder_train:
                raise AssertionError(
                    f"{agent_type} CamEncode.training={encoder.training} "
                    f"expected {expected_encoder_train}"
                )
            for name in _OFFICIAL_UNUSED_HEADS:
                module = getattr(encoder, name, None)
                if module is None:
            ...
            if self.freeze_backbone:
                if n_trainable_trunk != 0:
                    raise AssertionError(
                        f"{agent_type} EfficientNet trunk still trainable "
                        f"under freeze_backbone n={n_trainable_trunk}"
                    )
            elif mode and n_trainable_trunk == 0:
                raise AssertionError(
                    f"{agent_type} EfficientNet non-BN trunk has no trainable params"
                )
            for name in ("up1", "up2"):
                module = getattr(encoder, name)
                expected_up_train = bool(mode) and not self.freeze_backbone
                if bool(module.training) != expected_up_train:
                    raise AssertionError(
                        f"{agent_type}.{name}.training={module.training} "
                        f"expected {expected_up_train}"
                    )
                up_trainable = all(
                ...
                if self.freeze_backbone:
                    if any(param.requires_grad for param in module.parameters()):
                        raise AssertionError(
                            f"{agent_type}.{name} still trainable under freeze_backbone"
                        )
                elif not up_trainable:

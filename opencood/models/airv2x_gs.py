...
import opencood.models.uncertain_gaussian_modules.backbone3d as backbone3d
import opencood.models.uncertain_gaussian_modules.modality_fusion as modality_fusion
import opencood.models.uncertain_gaussian_modules.gaussian_lss as gaussian_lss
from opencood.models.common_modules.airv2x_base_model import Airv2xBase


class Airv2xGS(Airv2xBase):
    ...
    def _load_pretrained_backbones(self, model_cfg):
        ...
        if ckpt_3d and self.backbone_3d is not None:
            ...
            if os.path.exists(ckpt_3d):
                print(f"[Airv2xGS] 加载 backbone_3d 预训练权重: {ckpt_3d}")
                self.backbone_3d.load_pretrained_weights(
                    ckpt_3d, strict=False, freeze_pretrained=False
                )
            else:
                print(f"[Airv2xGS] 警告: backbone_3d 预训练权重文件不存在: {ckpt_3d}")
        ...
        if ckpt_2d and self.backbone_2d is not None:
            ...
            if os.path.exists(ckpt_2d):
                print(f"[Airv2xGS] 加载 backbone_2d 预训练权重: {ckpt_2d}")
                self.backbone_2d.load_pretrained_weights(ckpt_2d, strict=False)
            else:
                print(f"[Airv2xGS] 警告: backbone_2d 预训练权重文件不存在: {ckpt_2d}")
    ...
    def build_modality_fusion(self, model_info_dict):
        """构建模态融合模块"""
        if self.model_cfg.get('MODALITY_FUSION', None) is None:
            return None, model_info_dict
        
        modality_fusion_module = modality_fusion.__all__[self.model_cfg.MODALITY_FUSION.NAME](
            model_cfg=self.model_cfg.MODALITY_FUSION,
        )
        model_info_dict['module_list'].append(modality_fusion_module)
        return modality_fusion_module, model_info_dict

    def build_gaussian_lss(self, model_info_dict):
        """构建Gaussian LSS模块"""
        if self.model_cfg.get('GAUSSIAN_LSS', None) is None:
            return None, model_info_dict
        
        gaussian_lss_module = gaussian_lss.__all__[self.model_cfg.GAUSSIAN_LSS.NAME](
            model_cfg=self.model_cfg.GAUSSIAN_LSS,
        )
        model_info_dict['module_list'].append(gaussian_lss_module)
        return gaussian_lss_module, model_info_dict

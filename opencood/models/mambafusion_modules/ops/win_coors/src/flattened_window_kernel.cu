...
__global__ void gather_all_hit_points_kernel(
    const float* __restrict__ points_2d,     // [batch_size, num_view, grid_num, sample_num, 2]
    const int64_t* __restrict__ hit_indices, // [num_hits, 4] = [batch_idx, view_idx, grid_idx, sample_idx]
    float* __restrict__ hit_points_out,      // [num_hits, 3] = [u, v, view_idx]
    int64_t* __restrict__ point_ids_out,     // [num_hits]
    int64_t num_hits,
    int batch_size,
    int num_view,
    int grid_num,
    ...
    int64_t batch_idx = hit_indices[idx * 4 + 0];
    int64_t view_idx = hit_indices[idx * 4 + 1];
    int64_t grid_idx = hit_indices[idx * 4 + 2];
    int64_t sample_idx = hit_indices[idx * 4 + 3];

    int64_t points_2d_idx = ((((((batch_idx * num_view) + view_idx) * grid_num + grid_idx) * sample_num) + sample_idx) * 2);
    float hit_u = points_2d[points_2d_idx + 0];
    float hit_v = points_2d[points_2d_idx + 1];

    float hit_u_clamped = fminf(fmaxf(hit_u, -1.0f), 2.0f) + 1.0f;
    float hit_v_clamped = fminf(fmaxf(hit_v, -1.0f), 2.0f) + 1.0f;

    hit_points_out[idx * 3 + 0] = hit_u_clamped;
    hit_points_out[idx * 3 + 1] = hit_v_clamped;
    hit_points_out[idx * 3 + 2] = static_cast<float>(view_idx);
    point_ids_out[idx] = grid_idx * sample_num + sample_idx;
}

std::vector<at::Tensor> map_points_multi_view_cuda(
    at::Tensor points,
    at::Tensor lidar2image,
    at::Tensor image_aug_matrix,
    int batch_size,
    ...
    at::Tensor hit_indices = at::nonzero(map_mask_out);
    int64_t num_hits = hit_indices.size(0);

    auto float_options = points.options();
    auto long_options = points.options().dtype(at::kLong);
    at::Tensor hit_points_out = at::empty({num_hits, 3}, float_options);
    at::Tensor point_ids_out = at::empty({num_hits}, long_options);

    if (num_hits == 0) {
        return {hit_points_out, point_ids_out};
    }

    const int threads = 256;
    const int blocks = (num_hits + threads - 1) / threads;

    gather_all_hit_points_kernel<<<blocks, threads>>>(
        points_2d_out.data_ptr<float>(),
        hit_indices.data_ptr<int64_t>(),
        hit_points_out.data_ptr<float>(),
        point_ids_out.data_ptr<int64_t>(),
        num_hits,
        batch_size,
        num_view,
        grid_num,

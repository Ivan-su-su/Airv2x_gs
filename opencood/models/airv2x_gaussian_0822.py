...
class Airv2xGaussian0822(nn.Module):
    ...
    def __init__(self, args: Dict[str, Any]) -> None:
        ...
        self.rsu_depth_head_only = False
    ...
    def train(self, mode: bool = True) -> "Airv2xGaussian0822":
        ...
        if self.rsu_depth_head_only:
            self.highres.eval()
            self.heatmap_heads.eval()
            self.drone_height_embed.eval()
            self.drone_delta_head.eval()
            for agent_type, head in self.depth_heads.items():
                if agent_type == "rsu":
                    ...
                    head.eval()
            if mode:
                if not self.depth_heads["rsu"].training:
                    raise AssertionError("rsu DepthHead.training=False expected True")
                for agent_type, head in self.heatmap_heads.items():
                    if head.training:
                        raise AssertionError(
                            f"{agent_type} HeatmapHead.training=True expected False"
                        )
                for agent_type, fusion in self.highres.items():
                    if fusion.training:
                        raise AssertionError(
                            f"{agent_type} HighResFusion.training=True expected False"
                        )
                if self.depth_heads["vehicle"].training:
                    raise AssertionError("vehicle DepthHead.training=True expected False")
            self.frontend.assert_train_eval_state(mode)
            return self
        self.heatmap_heads.train(mode)
        if self.heatmap_only:
            self.highres.eval()
            self.depth_heads.eval()
            self.drone_height_embed.eval()
            self.drone_delta_head.eval()
        else:
            self.highres.train(mode)
            self.depth_heads.train(mode)
            self.drone_height_embed.train(mode)
            self.drone_delta_head.train(mode)
        if mode:
            for agent_type, head in self.heatmap_heads.items():

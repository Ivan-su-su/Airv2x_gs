    ...
    for batch_index in range(batch_size):
        ...
        for local_idx in range(n_this):
            ...
            for view_idx in range(int(n_views)):
                ...
                # RSU collated ``extrinsics`` acts as lidar-to-camera. Drone
                # ``project_box_to_image`` expects camera-to-lidar and inverts,
                # so pass inv(E) to recover lidar2cam = E.
                try:
                    ext_cam_to_lidar = np.linalg.inv(np.asarray(ext, dtype=np.float64))

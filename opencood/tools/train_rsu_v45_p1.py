# -*- coding: utf-8 -*-
"""Standalone RSU V45 P1 trainer. Reuses P1 loss / optimizer / logging.

Targets are built at ``output_stride=8`` (45x80). The V90 trainer and
90x160 path are not used. Frames without RSU cameras are skipped.
"""
...
from opencood.models.gaussian_modules_0822.rsu_v45_p1 import RsuV45P1
...
DEFAULT_YAML = (
    "opencood/hypes_yaml/airv2x/camera/gaussian_p1/rsu_v45_p1.yaml"
)
FAR_Z_M = 20.0
AGENT = "rsu"


def agent_has_cameras(ego: Dict[str, Any], agent: str) -> bool:
    """True iff this batch has camera ``imgs`` for ``agent``."""
    ...
    if not isinstance(cam, dict):
        return False
    imgs = cam.get("imgs")
    return torch.is_tensor(imgs) and int(imgs.numel()) > 0


def train_parser() -> argparse.Namespace:
    """CLI for RSU V45 P1 training and ``--verify``."""
    parser = argparse.ArgumentParser(description="RSU V45 P1 training")
...
def build_v45_heatmap_target(
    ...
    """SAM3 occupancy on the stride-8 RSU grid."""
...
def build_v45_depth_target(
    ...
    """Categorical RSU depth indices on the stride-8 grid."""
...
def forward_v45_loss(
    ...
    """RSU-only heatmap Focal + categorical depth. No Gaussian/det loss.
...
def validate_v45(
    ...
    with torch.no_grad():
        for _, batch_data in tqdm(
            ...
            ego = batch_data["ego"]
            ego["epoch"] = epoch
            if not agent_has_cameras(ego, AGENT):
                continue
...
def _make_synthetic_ego(
    ...
    """Synthetic RSU batch matching 360x640 + SAM3/depth channels."""
...
def verify_v45(hypes: Dict[str, Any], device: torch.device) -> None:
    ...
    if type(model).__name__ != "RsuV45P1":
    ...
    if pretrained and os.path.isfile(str(pretrained)):
        load_report = model.load_v90_rsu_weights(str(pretrained), device)
    else:
    ...
    if load_report:
        print(
            f"\n[6] V90 RSU load report: loaded={len(load_report['loaded'])} "
...
def main() -> None:
    """Train standalone RSU V45 P1, or run ``--verify``."""
    opt = train_parser()
    ...
    print(
        "P1 mode: standalone RSU V45 "
        f"(output_stride={stride}, expected 45x80). "
        "Loss = rsu heatmap + rsu depth. No Gaussian / Vehicle / Drone."
    )
    if test_mix_on:
        print(
    ...
    if pretrained and not opt.model_dir:
        model.load_v90_rsu_weights(str(pretrained), device)
    ...
    print("Starting RSU V45 P1 training...")
    ...
    if main_process:
        print(f"RSU V45 P1 training finished. Checkpoints saved to {saved_path}")

...
def compute_camera_world_z(params: Dict[str, Any], agent_type: str) -> np.ndarray:
    ...
    Uses the same camera key order as ``get_ex_intrinsic``. Valid at
    inference: no depth GT, semantic GT, or object boxes.

    Args:
        params: CAV metadata dict with ``odometry.ego_pos`` and per-camera
            ``cords``.
        agent_type: ``vehicle``, ``rsu``, or ``drone``.

    Returns:
        Float32 array of shape ``[Ncam]`` (world z in meters).
    """

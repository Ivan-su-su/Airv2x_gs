...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        if loaded is None:
            ...
            with open(lock_path, "w") as lock_file:
                ...
                loaded = self._try_load_scenario_index(cache_path, index_signature)
                if loaded is None:
                    self.scenario_database, self.len_record = self._build_scenario_index(
                        scenario_folders
                    )
                    self._save_scenario_index(
                        cache_path, index_signature, self.scenario_database, self.len_record
                    )
                    print(f"[index] wrote {cache_path}")
                else:
                    self.scenario_database, self.len_record = loaded
                    print(f"[index] loaded {cache_path} after lock")
        else:
            self.scenario_database, self.len_record = loaded
            print(f"[index] loaded {cache_path}")
    ...
    def _build_scenario_index(self, scenario_folders):
        """Walk scene folders once and record per-timestamp file paths."""
        scenario_database = OrderedDict()
        len_record = []
        for i, scenario_folder in enumerate(
            tqdm(
                scenario_folders,
                total=len(scenario_folders),
                desc="Loading %s dataset"
                % ("training" if self.train else "validation/testing"),
            )
        ):
            ...
            while True:
                ...
                if next(iter(scenario_database[i][first].items()))[1][
                    "agent_type"
                ] != self.ego_type:
                    scenario_database[i].move_to_end(first)
                else:
                    record_length = len(list(scenario_database[i][first].keys()))
                    if not len_record:
                        len_record.append(record_length)
                    else:
    ...
    def _try_load_scenario_index(cache_path, signature):
        """Return ``(scenario_database, len_record)`` if the cache matches."""
        ...
        if payload.get("signature") != signature:
            return None
        scenario_database = payload.get("scenario_database")
        len_record = payload.get("len_record")
        if scenario_database is None or not len_record:
            return None
        return scenario_database, len_record

    @staticmethod
    def _save_scenario_index(cache_path, signature, scenario_database, len_record):
        """Atomically dump the file-index cache."""
    ...
    def retrieve_base_data(self, idx, cur_ego_pos_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            if getattr(self, "use_lidar", True):
                ...
                data[cav_id]["lidar_np"] = np.zeros((0, 4), dtype=np.float32)
